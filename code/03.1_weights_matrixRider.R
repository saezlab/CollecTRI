...
GRN_prob <- map_dfr(1:nrow(GRN), function(j){
  print(j)
  x <- GRN[j,]
  if (any(names(ids) == x$target)){
    gene_id <- ids[names(ids) == x$target]
  } else {
    gene_id <- c("no gene id")
  }

  if (x$source %in% names(pwm_tfs) & gene_id %in% names(prom_sequence)){
...
## save results ---------------------------
saveRDS(GRN_prob, file.path("output", file.version, "03_weighted_strategies", paste0("matrixRider_res_", prom_length, "bp.rds")))
