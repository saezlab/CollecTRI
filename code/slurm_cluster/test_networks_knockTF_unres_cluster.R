...
bexample_url <- file.path('data',"bench", "knockTF_expr_dorotheaA_new.rds")
...
tibble(
  ...
  bexpr_loc = c(bexample_url, "data/bench/knockTF_expr_dorotheaABC_new.rds"), # benchmark data location
  bmeta_loc = c(bmeta_url, "data/bench/knockTF_meta_dorotheaABC_new.rds"), # metadata location
...
knockTF_exp_net <- rep(c("data/bench/knockTF_expr_NTNU_v1_unres.rds",
                         "data/bench/knockTF_expr_NTNU_v2_unres.rds",
                         "data/bench/knockTF_expr_NTNU_v2_dbTF_unres.rds"), each = 4)
knockTF_meta_net <- rep(c("data/bench/knockTF_meta_NTNU_v1_unres.rds",
                          "data/bench/knockTF_meta_NTNU_v2_unres.rds",
                          "data/bench/knockTF_meta_NTNU_v2_dbTF_unres.rds"), each = 4)

input_tibble <- input_tibble %>%
  mutate(bexpr_loc = c(input_tibble$bexpr_loc[1:2], knockTF_exp_net)) %>%
  mutate(bmeta_loc = c(input_tibble$bmeta_loc[1:2], knockTF_meta_net))
...
saveRDS(estimate@bench_res, paste0(output_path, "/estimate_knockTF_signed_test_networks_unres.rds"))
