...
ulm_CollecTRI <- IC50_ulm_CollecTRI %>%
  filter(Pval < 0.01)
...
ulm_CollecTRI <- ulm_CollecTRI %>%
  select(-Pval)
#  spread(TF, Activity)

names(ulm_CollecTRI) <- gsub("-","_", names(ulm_CollecTRI))

model_inp <- left_join(GDSC2_data_flt, ulm_CollecTRI,
...
save(model_inp_flt, file = "IC50_input_GDSC2_CollecTRI_tissue.RData")
...
anova_list <- function(df){
  tryCatch(Anova(df[[1]], type = "2"), error = function(e) NULL)
}

Anova(models_GDSC2_CollecTRI[1010][[1]],type = "2")
...
models_GDSC2_CollecTRI <- parLapply(cl, model_inp_flt, function(x) IC50_model(x))

anova_GDSC2_CollecTRI <- parLapply(cl, models_GDSC2_CollecTRI, function(x) Anova(x, type = "2"))

stopCluster(cl)

save(models_GDSC2_CollecTRI, file = "IC50_models_GDSC2_CollecTRI.RData")
save(anova_GDSC2_CollecTRI, file = "IC50_anova_GDSC2_CollecTRI.RData")

anova_adj <- lapply(anova_GDSC2_CollecTRI, function(x) x %>% mutate(Adjusted_P_Value = p.adjust(.$"Pr(>F)", method = "BH")) %>% rownames_to_column("Factor"))
...
models_coeff <- lapply(models_GDSC2_CollecTRI, function(x) (x$effects) %>% stack(.) %>%
