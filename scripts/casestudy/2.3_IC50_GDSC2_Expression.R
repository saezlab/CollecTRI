...
GDSC2_expr <- gather(GDSC2_expr, TF, Expression,ABL1:ZNF384)
...
save(model_inp_flt, file = "IC50_input_GDSC2_expr_tissue.RData")
...
models_GDSC2_expr <- parLapply(cl, model_inp_flt, function(x) lm(LN_IC50 ~ ., data = x))

anova_GDSC2_expr <- parLapply(cl, models_GDSC2_expr, function(x) Anova(x, type = "2"))

stopCluster(cl)

save(models_GDSC2_expr, file = "IC50_models_GDSC2_expr.RData")
save(anova_GDSC2_expr, file = "IC50_anova_GDSC2_expr.RData")
...
load("IC50_anova_005_GDSC2_expr.RData")
load("IC50_models_GDSC2_expr.RData")
...
res_all_GDSC2_expr <- res_all
save(res_all_GDSC2_expr, file ="IC50_GDSC2_expr_resAll.RData")
